...
import { Activity as ActivityIcon, CheckCircle, Zap, Clock, Brain } from 'lucide-react';
...
const DashboardOverview: React.FC = () => {
  ...
  const { jobs, stats } = useCronStore();

  const activeAgents = agents.filter(a => a.status === 'active').length;
  const enabledJobs = jobs.filter(j => j.enabled).length;
  const recentOk = jobs.filter(j => j.state?.lastStatus === 'ok').length;

  // Build activity from real cron job runs
  const recentActivity = jobs
    ...
    .map(j => ({
      id: j.id,
      title: j.name,
      description: `${j.state?.lastStatus === 'ok' ? '✅' : '⚠️'} ${j.agentId} · ${j.state?.lastDurationMs ? (j.state.lastDurationMs / 1000).toFixed(1) + 's' : ''}`,
      time: j.state?.lastRunAtMs ? new Date(j.state.lastRunAtMs).toLocaleTimeString('zh-TW', { hour: '2-digit', minute: '2-digit' }) : '',
    }));
    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      <header className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div className="flex flex-col gap-1">
          ...
          <p className="text-[13px] text-zinc-500">Real-time aidot ecosystem monitoring — {agents.length} agents, {jobs.length} scheduled tasks.</p>
        </div>
        <div className="flex items-center gap-2 px-3 py-1.5 rounded-full glass border-white/5 bg-white/[0.02]">
          <Brain size={14} className="text-blue-400" />
          <span className="text-[11px] font-bold text-zinc-300 uppercase tracking-widest">aidot Core v2.0</span>
        </div>
      </header>

      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          { label: 'Active Agents', value: String(activeAgents), icon: <Zap size={18} />, color: 'text-amber-400' },
          { label: 'Scheduled Jobs', value: String(enabledJobs), icon: <Clock size={18} />, color: 'text-blue-400' },
          { label: 'Jobs Succeeded', value: String(recentOk), icon: <CheckCircle size={18} />, color: 'text-emerald-400' },
          { label: 'Total Agents', value: String(stats?.agents.total || agents.length), icon: <ActivityIcon size={18} />, color: 'text-purple-400' },
        ].map((stat, idx) => (
          ...
          <GlassCard className="flex flex-col gap-6" delay={0.25}>
            ...
            <div className="h-[140px] -mx-4">
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={chartData}>
                  ...
                  <Tooltip contentStyle={{ backgroundColor: 'rgba(9, 9, 11, 0.9)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '12px', fontSize: '10px' }} />
            ...
            <div className="space-y-3">
              <div className="flex justify-between items-center">
                <span className="text-[11px] text-zinc-500 font-medium">Cron Success Rate</span>
                <span className="text-[12px] text-emerald-400 font-bold">{enabledJobs > 0 ? Math.round((recentOk / enabledJobs) * 100) : 0}%</span>
              </div>
              <div className="w-full h-1 bg-white/5 rounded-full overflow-hidden">
                <div className="h-full bg-emerald-500" style={{ width: `${enabledJobs > 0 ? (recentOk / enabledJobs) * 100 : 0}%` }} />
              </div>
            </div>
          </GlassCard>

          <GlassCard className="flex flex-col gap-4" delay={0.3}>
            <h3 className="text-[14px] font-semibold text-white">Recent Cron Runs</h3>
                ...
                <div key={act.id} className="flex gap-3 items-start group">
                  ...
                  <div className="flex flex-col gap-0.5 flex-1">
                    ...
                    <div className="flex justify-between">
                      <span className="text-[10px] text-zinc-600 truncate max-w-[120px]">{act.description}</span>
                      <span className="text-[9px] text-zinc-700">{act.time}</span>
                    </div>
                  </div>
                </div>
              )) : (
                <span className="text-[11px] text-zinc-600">No recent activity</span>
              )}
            </div>
